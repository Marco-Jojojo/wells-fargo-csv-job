...
public class CalcUnclearedBilledAmtDAOImpl implements CalcUnclearedBilledAmtDAO {
	...
	private static final String FIND_ALL = "SELECT cm.POLICY_NUMBER as REFERENCE_NUMBER, cm.SUBMISSION_NUMBER as SUBMISSION_NUMBER, cm.DIRECT_BILL_INVOICE as INVOICE_NUMBER, CASE WHEN "
			...
			+ "(SELECT top (1) s.STMT_DATE FROM SP_BILL_STMT_CTRL s WHERE s.POLICY_PREFIX_1 = cm.POLICY_PREFIX_1 AND s.POLICY_PREFIX_2 = cm.POLICY_PREFIX_2 AND s.POLICY_NUMBER = cm.POLICY_NUMBER "
			+ "AND s.POLICY_SUFFIX = cm.POLICY_SUFFIX AND s.INVOICE_NUMBER < 99999999 AND s.INVOICE_NUMBER > 0 ORDER BY s.INVOICE_NUMBER DESC) as INVOICE_DATE, (SELECT TOP (1) s.DUE_DATE FROM "
			+ "SP_BILL_STMT_CTRL s WHERE s.POLICY_PREFIX_1 = cm.POLICY_PREFIX_1 AND s.POLICY_PREFIX_2=cm.POLICY_PREFIX_2 AND s.POLICY_NUMBER = cm.POLICY_NUMBER AND s.POLICY_SUFFIX = cm.POLICY_SUFFIX "
			+ "AND s.INVOICE_NUMBER < 99999999 AND s.INVOICE_NUMBER > 0 ORDER BY s.INVOICE_NUMBER DESC) as DUE_DATE FROM COLLECTION_MASTER cm LEFT OUTER JOIN SP_BILL_STMT_CTRL spb ON "
			...
			+ "cm.DIRECT_BILL_INVOICE = spb.INVOICE_NUMBER WHERE cm.cleared_receivable = 'N' AND cm.O_COMMENT NOT LIKE '%OFFSET%' AND cm.DIRECT_BILL_INVOICE NOT IN (0, 99999999) GROUP BY "
			+ "cm.POLICY_NUMBER, cm.POLICY_PREFIX_1, cm.POLICY_PREFIX_2, cm.POLICY_SUFFIX, cm.SUBMISSION_NUMBER, cm.DIRECT_BILL_INVOICE";
