...
public class CalcClearedReceivableRecordsDAOImpl implements CalcClearedReceivableRecordsDAO {
	...
	public static final String FIND_ALL_CLEARED_RECEIVABLES = "SELECT wf.REFERENCE_NUMBER as REFERENCE_NUMBER "
	        + " ,wf.DUE_DATE as DUE_DATE " + " ,wf.INVOICE_NUMBER as INVOICE_NUMBER "
	        + " ,wf.INVOICE_DATE as INVOICE_DATE " + " ,wf.SUBMISSION_NUMBER as SUBMISSION_NUMBER "
	        + "FROM WF_MAM_SRC_FILE wf INNER JOIN  " + "	(SELECT  " + "		cm.SUBMISSION_NUMBER, "
	        + "		bill.DUE_DATE,  " + "		bill.STMT_DATE " + "		FROM COLLECTION_MASTER cm "
	        + "		 INNER JOIN SP_BILL_STMT_CTRL bill " + "		 ON cm.POLICY_PREFIX_1 = bill.POLICY_PREFIX_1 "
	        + "		 AND cm.POLICY_PREFIX_2 = bill.POLICY_PREFIX_2 "
	        + "		 AND cm.POLICY_NUMBER = bill.POLICY_NUMBER " + "		 AND cm.POLICY_SUFFIX = bill.POLICY_SUFFIX "
	        + "		 AND cm.DIRECT_BILL_INVOICE = bill.INVOICE_NUMBER " + "		 WHERE cm.CLEARED_RECEIVABLE = 'Y'  "
	        + "		 AND cm.DATE_CLEARED BETWEEN DATEADD(DAY, -60, GETDATE()) AND GETDATE() "
	        + "		 AND cm.DIRECT_BILL_INVOICE NOT IN (0, 99999999) " + "	 ) AS cm_bill "
	        + "ON cm_bill.SUBMISSION_NUMBER = wf.SUBMISSION_NUMBER " + " AND cm_bill.DUE_DATE = wf.DUE_DATE "
	        + " AND cm_bill.STMT_DATE = wf.INVOICE_DATE " + "WHERE wf.CLEARED_RECORD_SENT = 0 "
	        + "GROUP BY wf.REFERENCE_NUMBER " + " ,wf.SECONDARY_AUTH " + " ,wf.CONSOLIDATED_NAME " + " ,wf.DUE_DATE "
	        + " ,wf.AMOUNT_DUE " + " ,wf.INVOICE_NUMBER " + " ,wf.INVOICE_DATE " + " ,wf.STATUS_INVOICE "
