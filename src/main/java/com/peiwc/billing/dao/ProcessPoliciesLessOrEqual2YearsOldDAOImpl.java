...
public class ProcessPoliciesLessOrEqual2YearsOldDAOImpl implements ProcessPoliciesLessOrEqual2YearsOldDAO {
	...
	private static final String FIND_ALL_TWO_YEARS_OLD_POLICIES = ""
			+ "SELECT p.POLICY_NUMBER AS REFERENCE_NUMBER, p.SUBMISSION_NUMBER AS SUBMISSION_NUMBER , CASE WHEN MAX(spb.AMOUNT_DUE) < SUM(c.NET_PREMIUM_AMOUNT) "
			+ "THEN MAX(spb.AMOUNT_DUE) ELSE CASE WHEN SUM(c.NET_PREMIUM_AMOUNT) < 0 THEN MAX(spb.AMOUNT_DUE) ELSE SUM(c.NET_PREMIUM_AMOUNT) END END AS AMOUNT_DUE, "
			...
			+ "spb.STMT_DATE AS INVOICE_DATE, spb.DUE_DATE AS DUE_DATE FROM POLICY_MASTER p, SPR_FINANCIAL_FILE s, COLLECTION_MASTER c LEFT OUTER JOIN SP_BILL_STMT_CTRL spb "
			+ "ON c.POLICY_PREFIX_1 = spb.POLICY_PREFIX_1 AND c.POLICY_PREFIX_2 = spb.POLICY_PREFIX_2 AND c.POLICY_NUMBER = spb.POLICY_NUMBER AND c.POLICY_SUFFIX = spb.POLICY_SUFFIX "
			+ "AND c.DIRECT_BILL_INVOICE = spb.INVOICE_NUMBER WHERE p.SUBMISSION_NUMBER = s.SUBMISSION_NUMBER AND p.POLICY_PREFIX_1 = s.POLICY_PREFIX_1 AND "
			+ "p.POLICY_PREFIX_2 = s.POLICY_PREFIX_2 AND p.SUBMISSION_NUMBER = c.SUBMISSION_NUMBER AND p.POLICY_NUMBER = c.POLICY_NUMBER AND p.POLICY_PREFIX_1 = c.POLICY_PREFIX_1 AND "
			+ "p.POLICY_PREFIX_2 = c.POLICY_PREFIX_2 AND p.POLICY_SUFFIX = c.POLICY_SUFFIX AND p.WINNING_QUOTE_NUMBER = s.QUOTE_NUMBER  AND s.AGYDIRECT_BILL_CODE ='D'  "
			+ "AND p.EFFECTIVE_DATE < :today AND p.EFFECTIVE_DATE >= :twoYearsBefore AND c.CLEARED_RECEIVABLE='N' AND c.TRANSFER_FLAG='' AND c.O_COMMENT NOT LIKE '%OFFSET%' AND "
			+ "c.DIRECT_BILL_INVOICE NOT IN (0, 99999999) GROUP BY p.SUBMISSION_NUMBER, p.POLICY_NUMBER, c.POLICY_PREFIX_1, c.POLICY_PREFIX_2, c.POLICY_NUMBER, c.POLICY_SUFFIX, spb.STMT_DATE, "
			+ "spb.DUE_DATE, c.DIRECT_BILL_INVOICE";
